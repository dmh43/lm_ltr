...
class RelScore(nn.Module):
  ...
  def forward(self, query, document):
    ...
    pos_posterior = torch.sigmoid(torch.sum(query_embeds * document_tokens, 2) / len(document_tokens))
    neg_posterior = torch.sigmoid(torch.sum(query_embeds * neg_doc_tokens, 1) / len(neg_doc_tokens))
